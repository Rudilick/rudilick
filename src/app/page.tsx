  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="space-y-4">
        <Button className="w-full" onClick={() => openModal('free')}>🎧 영상 악보 변환 (무료 체험)</Button>
        <Button className="w-full" onClick={() => openModal('premium')}>💰 영상 모음 (유료 학습 콘텐츠)</Button>
      </div>
      ...
      <Dialog open={!!modal} onOpenChange={closeModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {modal === 'free' ? '무료 영상 변환 체험' : '유료 영상 학습 콘텐츠'}
            </DialogTitle>
          </DialogHeader>
          {modal === 'free' && (
